...
|---|---|---|
| ms/solve median | **28.80** (spread 0.0 %) | a build change is real past ±0.5 % |
| sol/s | **69.90** | derived; quoted because the pin cites it |
| SM / mem clock | 2610 / 10251 MHz | must match, or it is a different V/f point |
| board draw | **273.1 W** | context, not a gate — 265–281 W across seven sessions at identical clocks and work |
...
checking on any launch under ~1 ms: recovery's replay kernels (0.20 ms) are the next
candidates, and the fused rounds are not — their staging loops already keep eight
iterations of loads in flight.
