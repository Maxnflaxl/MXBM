...
| [Measured results, 2026-09-09](#the-pipe-census-at-the-140-w-clock-the-front-half-stays-alu-pipe-bound-the-back-half-leaves-dram) | **At the 140 W clock (1230 MHz pin) entry is still at the ALU roofline (98.8 %, ×2.24 stretch), r1/r2 still ALU-pipe-bound (75/69 %, barrier + long-scoreboard 32–36 %, DRAM ≤ 24 % of peak), and r3/r4 have left DRAM (55/54 % of peak, ×1.33/×1.47)** — under the cap the currency is instruction count in entry and r2, warp supply in r1/r2, latency in r3/r4, and bytes nowhere; also **both speculative-entry gates re-measured on the priority-stream form and stand** — rung 140 W +0.000 % (identical solve counts, spec arm 100 MHz lower), stock 210 W −0.42 % (t = 9); **and the 16 B entry checkpoint re-priced at 140 W is −1.9 % (t = 18), three times its stock loss** — the saved hash is ALU slots the cap already discounts, the added 0.27 GB of scattered writes is DRAM energy the cap charges as clock (variant arm 54 MHz lower); **and a resident round-2 block, 1.7 % of the solve at stock, is worth +0.15 % (t = 0.8) at 140 W** — warp supply is not a band currency, so the fifth-block, shared-narrowing and barrier families are closed under caps; **and round 3's word-0 hoist, null at stock, is −0.24 % (t = 3.7) at 140 W** — the round's chains are already overlapped across its 24 warps, the hoist adds issue, and under the cap issue is clock; **the stage attribution re-taken on the shipping kernels** (entry 2.69 / r1 4.72 / r2 8.14 / r3 8.09 / r4 3.94 / terminal 0.73 ms, 101 % of the 28.04 ms harness solve, every stage within 6 W of the cap, 10.21 GB compulsory to +0.1 %) after the pipeline harness was found still launching round 3 at the 8-word stride and failing its KAT; **round 3's mix deleted at 140 W is −1.39 % (t = 21)** against +0.024 ms at stock — under the cap instructions in the DRAM-bound rounds are full-price energy, so r3/r4 instruction deletion is a band lever class at ~0.7 × its share of the solve's instructions ([ledger](#round-3s-instructions-are-full-price-energy-at-140-w-deleting-its-mix-buys-14)); **geometry (17,0) is −3.17 % (t = 25) at 140 W on stock memory** — the sub-mask rescan it deletes is 2.6 % of the solve's instructions and the halved group is free under the cap — while the sub-pass kernel that deletes only the scan sector is −0.13 % ([ledger](#the-sub-mask-rescan-at-140-w-geometry-170-is-32-the-sub-pass-kernel-is-a-wash)) |
...
**Re-priced under a 140 W cap on the priority-stream build, 2026-09-09: −1.9 %, three
times the stock loss.** Same variant rebuilt (`MXBM_ENTRY16`), paired 4 + 4 × 60 s at
140 W on stock memory with the gate keeping speculation off, so the entry pass ran
standalone: 1136.5 → 1114.75 solves, t = 18, and the variant's arm held the core
**54 MHz lower** (1172 → 1118). The
[pipe census at that clock](#the-pipe-census-at-the-140-w-clock-the-front-half-stays-alu-pipe-bound-the-back-half-leaves-dram)
says why the sign does not flip: round 1's saved seventh hash and mix are ALU slots,
and under the cap an ALU slot is cheap because the pipe is the thing being clocked
down — while the 0.27 GB of extra scattered writes are DRAM energy at full price, and
the cap converts that energy straight into core clock for every round after. A lever
that trades bytes for instructions moves the wrong way under a cap, exactly as
[the 56 B record](#the-round-3-record-is-56-b-the-dead-word-goes-and-the-four-16-b-transactions-stay)
found from the other side. The entry record stays 8 B at every operating point.
...
### A resident round-2 block is worth nothing at 140 W: warp supply is not a band currency

The [pipe census](#the-pipe-census-r1-and-r2-are-22-points-under-the-alu-roofline-and-it-is-warp-supply)
priced a resident r2 block at 2.8 % of the round at stock, and the
[140 W pipe census](#the-pipe-census-at-the-140-w-clock-the-front-half-stays-alu-pipe-bound-the-back-half-leaves-dram)
shows the same warp-supply gap at the capped clock, wider. Whether that gap is worth
closing under the cap is one perturbation: a 4 KB dynamic shared-memory pad on round 2's
launch alone takes every r2 instantiation from four resident blocks to three (64
registers and 20.7–24.4 KB static already sit on the four-block cliff) and changes
nothing else. Positive control at stock: round 2 **7.97 → 8.44 ms (+5.9 %)**, the solve
26.9 → 27.3, drops 0.

Paired at 140 W on stock memory, 4 + 4 × 60 s, same binary in both arms:

| r2 blocks/SM | solves / 60 s | core |
|---|---|---|
| 4 (ships) | 1137.25 | 1171 MHz |
| 3 | 1135.50 | 1177 MHz |

**+0.15 %, t = 0.8 — null.** A perturbation worth 1.7 % of the solve at stock is worth
nothing under the cap. The fourth block's latency hiding costs the power it draws, and
at 140 W that power comes back as clock (+6 MHz on the three-block arm) for every round,
which is the [energy bound](#the-round-3-record-is-56-b-the-dead-word-goes-and-the-four-16-b-transactions-stay)
seen from the occupancy side, and the 2026-07-28
[occupancy-under-cap](#the-occupancy-optimum-does-move-under-a-cap--by-008--of-a-solve)
sign flip at full size.

What it closes for the band: the fifth r2 block (≤ 48 registers and ≤ 19456 B), every
shared-memory narrowing in r1/r2 whose value was that block, and the barrier-stall
family, whose currency was warp supply. Under a cap r1 and r2 are paid by instruction
count and nothing else. What is left for the band is instruction deletion that adds no
bytes, and the r3/r4 latency chains.

### Round 3's word-0 hoist at 140 W: −0.24 %, the bus was not the reason

The [replay entry](#the-replays-were-warp-serialised-on-their-hits-not-latency-bound)
closed the hoist on round 3's staging at stock because the round runs the bus at 77 %,
so its long-scoreboard cycles were queueing, not exposed round trips. At the
[140 W clock](#the-pipe-census-at-the-140-w-clock-the-front-half-stays-alu-pipe-bound-the-back-half-leaves-dram)
round 3 is at 55 % of DRAM peak with long-scoreboard still its top stall, which is
outside that closure's scope, so the register-hoist form was rebuilt: the first three
iterations' word-0 loads issue together ahead of the first filter, the remainder loop
unchanged, registers within 2, occupancy unchanged, KAT 3/3 × 15, drops 0. At stock it
reproduces the null (round 3 9.67 vs 9.68 ms).

Paired at 140 W on stock memory, 4 + 4 × 60 s: 1137.75 → 1135.00 solves, **−0.24 %,
t = 3.7**, the hoist arm 9 MHz lower.

The bus was not the reason the hoist did nothing; warp overlap was. Round 3 holds 24
warps per SM and each lane's chain is two or three loads deep, so at either clock the
memory system already has every chain in flight across warps, and moving one lane's
loads earlier changes the order it waits in, not the time. What the hoist adds — the
guards and the three-way unrolled prologue — is issue, and under the cap issue is
clock. The terminal round and the replays paid because their chains were per block
with the bus idle; round 3's are per lane with the SM full, and the lever does not
reach that shape at any operating point.

### Round 3's instructions are full-price energy at 140 W: deleting its mix buys 1.4 %

The [census](#the-instruction-census-69-of-every-instruction-is-siphash-and-that-bounds-the-rest)
measured `MXBM_ABL_MIX=3` — every `apply_mix` in round 3 deleted, 23.3 % of the round's
instructions and 2.0 % of the solve's, combine and store kept so the footprint is
unchanged — at **+0.024 ms** at stock: instructions in a DRAM-bound round were free. That
closure was priced in time at 285 W. Under a cap the currency is energy, and the same
ablation paired at 140 W on stock memory, 4 + 4 × 60 s: 1135.00 → 1150.75 solves,
**−1.39 %, −0.72 ms, t = 21**, the ablated arm 15 MHz higher. (Results wrong — the arm
is a timing probe, proven applied by its KAT failing; it recovers no solutions, which
spares it under 0.1 % of replays.)

So 2 % of the solve's instructions, in a round that runs the bus at 77 % at stock,
buy 1.4 % of the capped solve — about 0.7 of their share. At stock the round is
stall-bound and the mix's ALU work hides in DRAM waits; at 140 W the card is energy-bound,
every instruction issued is joules, and joules are clock. The same accounting that made
the 16 B entry checkpoint lose (bytes bought instructions) makes the round-3 and round-4
instruction budget a live band lever: those two rounds execute 23 G of the solve's 161.5 G
thread-instructions and none of it is SipHash, so each region the census named — mix
23 / 12 %, stage 14 / 15 %, walk + emit 10 / 17 %, address arithmetic 11 / 9 %,
singleton census 8 / 13 % of r3 / r4 — is now worth roughly 0.7 × its instruction share
of the solve at 140 W, where at stock every one of them measured null. The mix itself
cannot go (the spec needs it); what can is any instruction in these rounds that exists
to save bytes or latency, since neither is a band currency there.

### The sub-mask rescan at 140 W: geometry (17,0) is −3.2 %, the sub-pass kernel is a wash

At (16,1) every block scans its whole bucket's word 0 and keeps half. From the census that
scan loop and its filter are 2.9 / 1.9 / 6.6 / 9.9 % of rounds 1–4's instructions — 4.18 G,
2.6 % of the solve, half of it on elements the block discards — plus one duplicated 32 B
sector per element per round. Two mechanisms delete it, both closed at stock because they
trade parallelism, and [warp supply](#a-resident-round-2-block-is-worth-nothing-at-140-w-warp-supply-is-not-a-band-currency)
is not a band currency. Both paired at 140 W on stock memory, 4 + 4 × 60 s, against the
shipping build:

| arm | what it deletes | what it costs | solves / 60 s | result |
|---|---|---|---|---|
| `MXBM_BB=17 MXBM_SM=0` (runtime) | the rescan in every round | twice the blocks at half the group | 1135.25 → 1171.25 | **−3.17 %, −1.62 ms, t = 25** |
| sub-pass kernel at (16,1) | the duplicated scan sector; keys read once into shared | the two sub-masks run in sequence in one block | 1132.25 → 1133.75 | −0.13 %, t = 4.2 |

The sub-pass arm ran 28 MHz higher and finished no sooner: the sectors it saves come back
as clock, and the serialised sweeps spend that clock. (17,0) deletes the same scan and
keeps the sub-masks concurrent, and under the cap the halved group costs nothing — a
block of 132 elements issues half the instructions of a block of 264, and idle lanes
draw no energy. At stock the same geometry loses 10.9 %; on the memory rung on the 08-15
build it was null at 140 W and −7.4 % at 160. So the low-power gate at 130 W is below
its crossover on stock memory on this build, and the crossover on the rung is unmeasured
on it. Spec entry and match-first were both off in both arms (140 W is above one gate and
below the other), so the arm is the geometry alone.
