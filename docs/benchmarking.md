...
re-measured 2026-08-16 on the singleton-filter build, six 120 s runs, **compute GPU
headless**):

| quantity | reference | gate |
|---|---|---|
| ms/solve median | **29.10** (spread 0.0 %) | a build change is real past ±0.5 % |
| sol/s | **69.00** (spread 0.1 %) | derived from the above; quoted because the pin is what the headline cites |
