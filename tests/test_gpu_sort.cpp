// Phase-D3: isolate + verify the sort-based collision-finder building blocks
// (kernels/opencl/sort.cl) against CPU oracles. Radix correctness (stable order +
// prefix-sum/reorder) and the run-scan emission offsets are the crux, pinned here
// on small/medium N plus a 2^25 pipeline-scale case:
//   1. scan_block/scan_add                == std::exclusive_scan
//   2. collision_count+emit               == CPU group-by-key C(m,2) emission
//   3. radix_sort_pairs_tiled (6x4-bit)   == std::stable_sort by 24-bit key
...
// Tiled 4-bit radix driver (the production sort): 6 passes, TILE=WG*IPT, 16-bin
// bin-major hist. In-place contract (result ends back in `buf`).
...
// --- test 2: collision run-scan emission ---------------------------------------
static void test_collision(Runtime& rt, cl_program prog) {
    ...
    for (uint32_t n : {2u, 300u, 4096u, 100000u}) {
        ...
        radix_sort_pairs_tiled(rt, prog, buf.get(), n);
...
int main() {
    ...
    // Tiled radix correctness across scales (incl heavy collisions + partial
    // tiles) plus a 2^25 pipeline-scale timing number.
    {
        section("tiled 4-bit radix == std::stable_sort by 24-bit key");
