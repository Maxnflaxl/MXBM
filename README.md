...
| **Smaller footprint** | A CUDA floor of **2.94 GiB** against a 3 GB design target ([HW_REQUIREMENTS.md](docs/HW_REQUIREMENTS.md)) — the target met on footprint, from 2.4× over at the start, and the 4 GB card class reached. The [24 B quad record](docs/performance-research.md#the-quad-record-29--footprint-and-the-byte-prize-does-not-survive-re-derivation), the [survivor-sized back-ref row](docs/performance-research.md#measured-results-2026-08-12), the implicit-bits record, the [dense-cap rungs](docs/performance-research.md#the-reach-composition-built-the-cuda-floor-moves-440--403-gib) and the [32 B octo record](docs/performance-research.md#the-octo-record-the-last-thing-round-3s-output-had-left-to-give) got it there; OpenCL carries only the first two and floors at 4.40. What is left is no longer the records but the **1.03 GiB of back-reference rows**, which the octo rung may make largely redundant |
