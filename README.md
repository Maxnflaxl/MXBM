...
| **Smaller footprint** | A CUDA floor of **4.03 GiB** against a 3 GB design target ([HW_REQUIREMENTS.md](docs/HW_REQUIREMENTS.md)) — 1.3× over, from 2.4× at the start. The [24 B quad record](docs/performance-research.md#the-quad-record-29--footprint-and-the-byte-prize-does-not-survive-re-derivation), the [survivor-sized back-ref row](docs/performance-research.md#measured-results-2026-08-12), the implicit-bits record and the [dense-cap rungs](docs/performance-research.md#the-reach-composition-built-the-cuda-floor-moves-440--403-gib) got it there; OpenCL carries only the first two and floors at 4.40, on *both* backends since the [record-set split](docs/performance-research.md#the-record-set-split-opencl-reaches-cudas-57-gib-floor-and-gets-faster-doing-it) lifted its single-allocation ceiling. The last 1 GiB is two named but unbuilt levers: an octo round-3 record, and moving the back-ref rows off the card |
