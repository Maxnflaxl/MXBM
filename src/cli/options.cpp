...
std::string usage_text() {
    return
        ...
        "                         --pl value (needs root, ~20 min, no pool): a coarse pass\n"
        "                         across the card's band, ~10 W steps around the knee, then\n"
        "                         the card's low memory rung at the capped points -- so the\n"
        "                         verdict can also say when to add --mclk. Stored per card;\n"
        "                         apply the wattage with --pl auto.\n"
